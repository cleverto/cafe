...
import RotacionForm from "./RotacionForm";
import { useLocation } from "react-router-dom";

const CargoRegistrar = () => {
  ...
  const modulo = async (id) => {
    ...
    await Axios.post(window.globales.url + "/rotacion/modulo", _datos)
      .then((res) => {
        if (res.data.rpta === "1") {
          formik.setFieldValue("operacion", "1");
          formik.setFieldValue("rotacion", res.data.items.rotacion);
  ...
  const guardar = async (data) => {
    ...
    await Axios.post(window.globales.url + "/rotacion/guardar", _datos)
      .then((res) => {
        if (res.data.rpta === "1") {
