...
import TrabajadorVinculoRegistrar from "./TrabajadorVinculoRegistrar";
import TrabajadorMain from "./TrabajadorMain";

const TrabajadorVinculo = () => {
  ...
  const get_lista = async () => {
    const res = await Axios.post(
      window.globales.url + "/trabajadorvinculo/lista"
    );
  ...
  const modifica = (obj) => {
    setRowdata((prevData) =>
      prevData.map((item) =>
        String(item.id_trabajador_vinculo) === String(obj.id_trabajador_vinculo)
          ? { ...item, ...obj }
          : item
      )
    );
  ...
  const eliminar = async (id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Axios.post(window.globales.url + "/trabajadorvinculo/eliminar", _datos)
          .then((res) => {
            if (res.data.rpta === "1") {
              setRowdata((prevData) =>
  ...
  const get_columns_proceso = () => {
    setColumns([
      ...
      },
      {
        ...
        selector: (row) => row.tipo_resolucion,
        sortable: true,
        reorder: true,
        wrap: true,
        width: "10rem",
...
export default TrabajadorVinculo;
