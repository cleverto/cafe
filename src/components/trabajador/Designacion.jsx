...
const Designacion = () => {
  ...
  const get_lista = async () => {
    const res = await Axios.post(window.globales.url + "/designacion/lista");
  ...
  const eliminar = async (id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Axios.post(window.globales.url + "/designacion/eliminar", _datos)
          .then((res) => {
            if (res.data.rpta === "1") {
              setRowdata((prevData) =>
                prevData.filter((row) => row.id_designacion !== id)
              );
            }
          })
          .catch((error) => {
    ...
    setColumns([
      {
        id: 1,
        name: "Código",
        selector: (row) => row.id_designacion,
        sortable: true,
        reorder: true,
        width: "6rem",
      },
          ...
          <>
            <Dropdown className="hide-split-after ">
              ...
              <Dropdown.Menu>
                <Dropdown.Item
                  as={Link}
                  to={`../designacion/editar?id=${row.id_designacion}`}
                >
                  <i className="bi bi-pencil-fill me-2"></i>Modificar
                </Dropdown.Item>
                <Dropdown.Item onClick={(e) => eliminar(row.id_designacion)}>
  ...
  const componente = (
    <>
      <div className="d-flex justify-content-between">
        <div className="">
          <h2>Designacion</h2>
          <p>Designación y/o encargaturas</p>
        </div>
        <div className="pt-4">
          <Link to="/designacion/nuevo">
